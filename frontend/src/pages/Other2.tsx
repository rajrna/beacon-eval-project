...
export function Programs() {
  ...
  const handleCreate = async () => {
    await create.mutateAsync({
      ...
      tuition_per_credit: form.tuition_per_credit
        ? parseFloat(form.tuition_per_credit)
        : undefined,
      total_credits: form.total_credits
        ? parseInt(form.total_credits)
        : undefined,
      ...
      <div className="mb-4">
        ...
        {!institutionId && (
          <p className="text-xs text-gray-600 mt-1">
            Select an institution to see its
            programs
          </p>
        )}
      </div>
          ...
          {!programs?.items?.length ? (
            <div className="px-6 py-8">
              <EmptyState
                ...
                description={
                  institutionId
                    ? "Create the first program for this institution."
                    : "Choose an institution from the dropdown above."
                }
                action={
                  institutionId ? (
                    <Button
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Select
              label="Term Calendar"
              value={form.term_calendar}
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  term_calendar: e.target.value,
                }))
              }
              options={[
                {
    ...
    <div>
      ...
      {!programId && (
        <p className="text-xs text-gray-600 mb-4">
          Select a program to see its agents
        </p>
      )}
      {isLoading ? (
        <Spinner />
          ...
          {!agents?.items?.length ? (
            <div className="px-6 py-8">
              <EmptyState
                ...
                description={
                  programId
                    ? "Create the first agent for this program."
                    : "Choose an institution and program above."
                }
                action={
                  programId ? (
                    <Button
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            placeholder="MBA Enrollment Bot"
          />
          <Select
            label="Role"
            value={form.role}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  owner_team: e.target.value,
                }))
              }
              placeholder="Enrollment Engineering"
            />
            <Input
              label="Owner Email"
          ...
          <p className="text-xs text-gray-500">
            After creating the agent, go to its
            detail page to add a system prompt
            version.
          </p>
          <div className="flex justify-end gap-3 pt-2">
            <Button
...
export function Datasets() {
  ...
  const { data: datasets, isLoading } =
    useDatasets(programId);
  ...
  return (
    <div>
      ...
      {!programId && (
        <p className="text-xs text-gray-600 mb-4">
          Select a program to see its datasets
        </p>
      )}
      {isLoading ? (
        <Spinner />
          ...
          {!datasets?.items?.length ? (
            <div className="px-6 py-8">
              <EmptyState
                ...
                description={
                  programId
                    ? "Create the first dataset for this program."
                    : "Choose an institution and program above."
                }
                action={
                  programId ? (
              ...
              <Tbody>
                {judges.items.map((j) => (
                  ...
                  >
                    ...
                    <Td>
                      <code className="text-xs bg-white/5 px-1.5 py-0.5 rounded text-gray-400">
                        {j.slug}
                      </code>
                    </Td>
                    <Td>
...
import {
  useInstitutions as useInstitutions2,
  usePrograms as usePrograms2,
  useAgents as useAgents2,
  useTraces,
} from "@/lib/api/hooks";
...
export function TraceBrowser() {
  ...
  const { data: institutions } =
    useInstitutions2();
  const { data: programs } = usePrograms2(
    institutionId,
  );
  ...
  return (
    <div>
      <PageHeader
        title="Trace Browser"
        description="Production traces from student agent interactions"
      />
        ...
        <Card>
          {!traces?.items?.length ? (
            <div className="px-6 py-8">
              <EmptyState
                title="No traces"
                description="Select an agent version to browse traces."
              />
            </div>
          ) : (
            <Table>
  ...
  return (
    <div>
      ...
      <div className="max-w-lg space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>About Beacon</CardTitle>
          </CardHeader>
          <CardContent>
