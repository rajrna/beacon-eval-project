...
export function Shell({
  ...
  const { data: crisisQueue } = useSmeQueue(
    "queued",
    "crisis",
  );
  const crisisCount =
    crisisQueue?.items?.length ?? 0;
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-white border-b flex items-center px-4 lg:px-6 gap-4">
          ...
          {crisisCount > 0 && (
            <a
              href="/sme/queue?priority=crisis"
              className="flex items-center gap-1.5 text-red-600 text-sm font-medium animate-pulse"
            >
